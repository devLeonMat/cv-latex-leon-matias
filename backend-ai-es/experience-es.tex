...
\begin{cvItems}
  \cvItem{Arquitectura backend de una \textbf{plataforma SaaS} cloud-native para una aseguradora
    de salud en EE.UU. — microservicios para registro de afiliados, gestión de pólizas,
    cobertura de beneficios y flujos de autorización de atención médica.}
  \cvItem{Microservicios \textbf{AWS} (Lambda, EKS, SQS) con \textbf{Java / Spring Boot};
    contenedorizados con Docker y orquestados con Kubernetes.}
  \cvItem{Backend de \textbf{red de clínicas y prestadores}: APIs REST para onboarding de
    establecimientos, credencialización, directorio de proveedores y catálogo de servicios médicos.}
  \cvItem{Arquitectura \textbf{event-driven} con \textbf{Apache Kafka} para procesamiento asíncrono
    de eventos de autorización de atención, notificaciones de siniestros y triggers de facturación.}
  \cvItem{APIs REST de registro de afiliados y clínicas con validación de datos, capas de
    persistencia alineadas a HIPAA y backends \textbf{PostgreSQL} / \textbf{MongoDB}.}
  \cvItem{\textbf{Desarrollo asistido por IA} (Claude Code, GitHub Copilot, Cursor) para scaffolding
    backend y revisiones automatizadas; liderazgo de equipo distribuido en Agile con CI/CD en
    GitHub Actions.}
\end{cvItems}
